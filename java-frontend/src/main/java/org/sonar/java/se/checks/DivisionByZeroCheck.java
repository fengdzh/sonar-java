...
public class DivisionByZeroCheck extends SECheck {
  ...
  private class PreStatementVisitor extends CheckerTreeNodeVisitor {
    ...
    private void reportIssue(Tree tree, SymbolicValue denominator, Symbol rightOpSymbol) {
      ...
      Set<List<JavaFileScannerContext.Location>> flows = FlowComputation.flow(context.getNode(), denominator, domains, rightOpSymbol).stream()
